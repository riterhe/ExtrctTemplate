...
public class TemplateData {
	...
	private static String outputPath= "test";
	...
	public ArrayList<String> getBox(Element basicInfo) {
		...
		if ((!dtList.isEmpty()) && (!ddList.isEmpty())) {
			for (int i = 0; i < dtList.size(); i++) {
				String property = NoiseReduction.removeAllSymbol(dtList.get(i).text());//split("[ 、；,，;]|（》《）")
				String[] values = NoiseReduction.removePunctuation(ddList.get(i).text(), " ").split(" ");
